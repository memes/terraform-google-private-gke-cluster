"""Test fixture for standard GKE cluster with fixed node-pool configuration."""
...
from .gke_standard_assertions import (
...
FIXTURE_NAME = "root-fixed-pool"
FIXTURE_LABELS = {
    "fixture": FIXTURE_NAME,
}
FIXED_NODE_POOL_CONFIG = {
...
def vpc_fixture_output(
    ...
    """Create VPC and bastion for test case."""
    with run_tofu_in_workspace(
...
def fixture_output(
    ...
    """Create standard GKE cluster for test case."""
...
def test_node_pools(cluster: container_v1.Cluster, fixture_name: str, service_account: str) -> None:
    ...
    for node_pool in cluster.node_pools:
        ...
        assert re.match(r"^fixed", node_pool.name)
        assert node_pool.config is not None
        assert node_pool.config.service_account == service_account
        assert node_pool.config.metadata is not None
        assert node_pool.config.metadata["node_pool"] == "fixed"
        assert node_pool.config.metadata["cluster_name"] == fixture_name
        assert node_pool.config.metadata["disable-legacy-endpoints"] == "true"
        labels = node_pool.config.labels
        assert labels is not None
        assert labels["node_pool"] == "fixed"
        assert labels["cluster_name"] == fixture_name
        assert_node_config(config=node_pool.config, settings=FIXED_NODE_POOL_CONFIG)
        assert node_pool.initial_node_count == 1
        assert node_pool.status == container_v1.NodePool.Status.RUNNING
        assert node_pool.autoscaling is not None
        assert not node_pool.autoscaling.enabled
        assert node_pool.management is not None
        assert node_pool.management.auto_repair
        assert node_pool.management.auto_upgrade
