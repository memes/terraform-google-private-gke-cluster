"""Test fixture for Autopilot GKE cluster with public endpoint configuration."""
...
def test_output_values(fixture_output: dict[str, Any], project_id: str, region: str, fixture_name: str) -> None:
    ...
    endpoint_url = fixture_output["public_endpoint_url"]
    assert endpoint_url
    url = urllib.parse.urlparse(endpoint_url)
    assert url
    assert url.scheme == "https"
...
def test_control_plane_endpoints_config(cluster: container_v1.Cluster) -> None:
    ...
    assert not cluster.control_plane_endpoints_config.dns_endpoint_config.allow_external_traffic
    assert cluster.control_plane_endpoints_config.ip_endpoints_config.enabled
    assert cluster.control_plane_endpoints_config.ip_endpoints_config.enable_public_endpoint
    assert cluster.control_plane_endpoints_config.ip_endpoints_config.global_access
    ...
    assert len(cluster.control_plane_endpoints_config.ip_endpoints_config.authorized_networks_config.cidr_blocks) > 0
