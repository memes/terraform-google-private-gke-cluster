"""Test fixture for Autopilot GKE cluster with DNS and IP address endpoint configuration."""
...
def test_output_values(fixture_output: dict[str, Any], project_id: str, region: str, fixture_name: str) -> None:
    ...
    assert not url.port
    private_ip_endpoint_url = fixture_output["private_ip_endpoint_url"]
    assert private_ip_endpoint_url
    url = urllib.parse.urlparse(private_ip_endpoint_url)
    assert url
    assert url.scheme == "https"
    assert not url.port
...
# Override teh default assertion for control-plane endpoints
def test_control_plane_endpoints_config(cluster: container_v1.Cluster) -> None:
    ...
    assert cluster.control_plane_endpoints_config
    assert cluster.control_plane_endpoints_config.dns_endpoint_config.endpoint
    assert cluster.control_plane_endpoints_config.dns_endpoint_config.allow_external_traffic
    assert cluster.control_plane_endpoints_config.ip_endpoints_config.enabled
    assert not cluster.control_plane_endpoints_config.ip_endpoints_config.enable_public_endpoint
    assert not cluster.control_plane_endpoints_config.ip_endpoints_config.global_access
    assert cluster.control_plane_endpoints_config.ip_endpoints_config.private_endpoint
    assert cluster.control_plane_endpoints_config.ip_endpoints_config.authorized_networks_config
    assert cluster.control_plane_endpoints_config.ip_endpoints_config.authorized_networks_config.cidr_blocks
    assert len(cluster.control_plane_endpoints_config.ip_endpoints_config.authorized_networks_config.cidr_blocks) == 1
...
def test_proxied_ip_endpoint_url(
    fixture_output: dict[str, Any],
    bastion_proxy_url: str,
) -> None:
    """Verify that access to private IP address Kubernetes API proxied through a VPC bastion is successful."""
    endpoint_url = fixture_output["private_ip_endpoint_url"]
    assert endpoint_url
